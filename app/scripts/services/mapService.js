			...
			function(DailyResortStatus) {
				...
				var CreateMap = {
					updateMap: function(resort) {
						...
						switch (resort) {
							...
							case 'Vail':
								...
								DailyResortStatus.getVailStatus()
									.then(function(data) {
										for (var i = 1; i < data.data.length; i++) {
											var trail = data.data[i];
					...
					createMap: function(resort) {
						...
						console.log('local data is ' + resort + ', keyName: ' + keyName + ', dailyJSON: ' + dailyJSON + ', mapCenter: ' + mapCenter);
